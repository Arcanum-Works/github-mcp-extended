...
- **checks_write** - Create or update a check run
  - **Required OAuth Scopes**: `repo`
  - `check_run_id`: Numeric check run ID. Required for 'update_check_run'. (number, optional)
  - `completed_at`: When the check run finished, as an RFC3339 timestamp. Only meaningful once the run is completed. (string, optional)
  - `conclusion`: Final result of the check run. Required when status is 'completed', and setting it implies completion. (string, optional)
  - `details_url`: URL the check run links to from the GitHub UI, e.g. the build output. (string, optional)
  - `external_id`: Caller-defined identifier for the run, echoed back by GitHub. (string, optional)
  - `head_sha`: Commit SHA the check run reports on. Required for 'create_check_run'. (string, optional)
  - `method`: The method to execute:
